...
test('compose an epic', async ({ page, editorPage }) => {
    ...
    expect(await editorPage.root.innerHTML()).toEqual('<p>The Whale</p><p><br></p>');
    await page.keyboard.press('Enter');
    ...
    await Promise.all(Array(CHAPTER.length)
        .fill(0)
    ...
    expect(await editorPage.root.locator('p').nth(2).innerHTML()).toBe('AB<strong>C</strong>DA');
